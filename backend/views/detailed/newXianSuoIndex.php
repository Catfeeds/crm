        ...
        <div class="table-responsive">
            ...
            <div style="margin: 10px 10px 10px 0px;">
                <form id="form1" class="mb-md" action="new-xian-suo" method="get">
                    ...
                    <input class="btn btn-primary pull-left mb-15" value="导出列表" type="button" onclick="butCheck(1)">
                    <div class="col-sm-6 t-r pull-right mb-15">
                            <div class="pull-right">
                                <input class="btn btn-primary btn-sm pull-right" value="查询" type="button" onclick="butCheck(0)" >
                                <div class="col-sm-9 col-md-9 pull-right">
                                    <input class="form-control mr-15" type="text" style="width:200px;" id="keyword" name="keyword" value="<?php echo $get['keyword'];?>" placeholder="创建人/姓名/手机号码/顾问">
                                </div>
                            </div>
                    </div>
                </form>
                        ...
                        <tr style="font-size: 14px;">
                            ...
                            <td>
                                ...
                                if ($get['type'] == 0 || $get['type'] == 4)
                                    echo empty($v['create_time']) ? '--' : $v['create_time'];
                                else if ($get['type'] == 1)
                                    echo empty($v['assign_time']) ? '--' :$v['assign_time'];
                                ?>
                            </td>
                            <td>
                                <?php echo empty($v['create_person_name']) ? '--' : $v['create_person_name'] ?>
                            </td>
                            <td>
                            ...
                            <td><?php echo empty($v['des']) ? '--' : $v['des'] ?></td>
                            <td><?php echo empty($v['last_view_time']) ? '--' : $v['last_view_time'] ?></td>
                            <td><?php
                                if ($get['id'] == 0 && empty($v['shop_id'])) echo "未分配";
                                else echo $v['statusDes'];
                                ?></td>
