...
class Auth extends HttpBasicAuth
{
    /**
     * @inheritdoc
     */
    public function authenticate($user, $request, $response)
    {
        ...
        {
            $arrCache = \Yii::$app->cache->get(md5($rData['access_token']));
        }
        ...
        {
            $strLogFile = \Yii::getAlias('@frontend/runtime/old_login.log');//登录校验log记录，便于查找app登录状态无故丢失的原因
    ...
    {
        $strLogFile = \Yii::getAlias('@frontend/runtime/login_error_sales.log');//登录校验log记录，便于查找app登录状态无故丢失的原因
        $strLog = '[' . date('Y-m-d H:i:s') . '] : ';
        ...
        if (empty($rData) && empty($pData)) {
            ...
            die(json_encode(['code' => 401, 'message' => '参数不全', 'data' => null]));
        }

        if ($this->auth) {
            $phone = $pData['phone'];
            $password = $pData['password'];
            $selectShopId = isset($pData['shop_id']) ? $pData['shop_id'] : 17;//登陆时传过来的shop_id
            $selectRoleId = isset($pData['role_id']) ? $pData['role_id'] : 64;//登录时传过来 角色ID
            if (!empty($phone) && !empty($password) && !empty($selectShopId) && !empty($selectRoleId)) {
                $result = call_user_func($this->auth, $phone, $password, $selectShopId, $selectRoleId);
                if (is_object($result)) {
                    $identity = $result;
                    file_put_contents($strLogFile, $strLog . "$phone 登录成功;\n", FILE_APPEND);
                    $arrSave = ['id' => $identity->id, 'shop_id' => $selectShopId, 'role_id' => $selectRoleId];
                    \Yii::$app->cache->set(md5($identity->access_token), $arrSave);
                    $user->switchIdentity($identity);
                } else {
                    file_put_contents($strLogFile, $strLog . "$phone 用户名或密码错误，或者没权限登陆;\n", FILE_APPEND);
                    die(json_encode(['code' => 403, 'message' => $result, 'data' => null]));
                }
                return $identity;
            }
        } else {
            if (!isset($rData['access_token'])) {
                ...
                die(json_encode(['code' => 401, 'message' => 'access_token no find', 'data' => null]));
            }
            $accessToken = $rData['access_token'];
            $arrCache = \Yii::$app->cache->get(md5($accessToken));
            $id = intval($arrCache['id']);
            if ($id) {
                $identity = User::findOne($id);
                if ($identity === null) {
                    file_put_contents($strLogFile, $strLog . "access_token 缓存的id无效了;\n", FILE_APPEND);
                    die(json_encode(['code' => 401, 'message' => '请重新登录', 'data' => null]));
                }
    ...
    {
        $strLogFile = \Yii::getAlias('@frontend/runtime/login_error_sales_old.log');//登录校验log记录，便于查找app登录状态无故丢失的原因
        $strLog = '[' . date('Y-m-d H:i:s') . '] : ';
        header("Content-Type:application/json; charset=UTF-8");
        $rData = json_decode($request->post('r'), true);
        $pData = json_decode($request->post('p'), true);
